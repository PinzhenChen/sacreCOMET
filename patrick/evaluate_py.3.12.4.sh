...
log_file=log_comet2.2.2-wmt22da.txt
rm -f ${log_file}
touch ${log_file}
...
for yy in 23 ; do
    ...
    for lang in de zh ; do
        ...
        hyp_file_prefix=data/wmt${yy}/en-${lang}/wmt${yy}.en-${lang}.*
        ...
        # X->en
        src_file=data/wmt${yy}/${lang}-en/wmt${yy}.${lang}-en.${lang}
        ref_file=data/wmt${yy}/${lang}-en/wmt${yy}.${lang}-en.en
        if [[ $(wc -l ${src_file} | cut -d" " -f1) -eq 0 ]] || [[ $(wc -l ${ref_file} | cut -d" " -f1) -eq 0 ]]; then
            continue
        fi

        hyp_file_prefix=data/wmt${yy}/${lang}-en/wmt${yy}.${lang}-en.*
        hyp_files=$(ls ${hyp_file_prefix} | xargs -n 1 -I {} bash -c '[[ $(wc -l {} | cut -d" " -f1) -gt 0 ]] && echo {}')
